...
export function NoteItem({ note, isActive }: NoteItemProps) {
  ...
  return (
    <Link
      ...
      className={`block border-b border-stone-100 dark:border-stone-700/30 px-3.5 py-2.5 transition-colors ${
        isActive
          ? 'bg-white dark:bg-stone-700'
          : 'bg-transparent hover:bg-stone-50 dark:hover:bg-stone-700/50'
      } ${isActive ? 'border-l-2' : 'border-l-2 border-l-transparent'}`}
      style={isActive ? { borderLeftColor: accentColor } : undefined}
    >
      <div className="text-xs text-stone-400 dark:text-stone-500 mb-1">{formatDate(note.updated_at)}</div>
      <div className="text-xs font-medium text-stone-800 dark:text-stone-200 mb-1.5 truncate">
        {note.title}
      </div>
      <div className="text-xs text-stone-400 dark:text-stone-500 mb-2 line-clamp-1">{preview}</div>
      <div
